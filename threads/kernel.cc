...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
	    ...
	    ASSERT(i + 1 < argc);
	    timequant1 = atoi(argv[i + 1]);
	    i++;
	} else if (strcmp(argv[i], "-q2") == 0) {
	    ASSERT(i + 1 < argc);
	    timequant2 = atoi(argv[i + 1]);
	    i++;
	} else if (strcmp(argv[i], "-q3") == 0) {
	    ASSERT(i + 1 < argc);
	    timequant3 = atoi(argv[i + 1]);
	    i++;
	} else if (strcmp(argv[i], "-q4") == 0) {
	    ASSERT(i + 1 < argc);
	    timequant4 = atoi(argv[i + 1]);
	    i++;
	} 
...
//----------------------------------------------------------------------
static int
ReqobjCompare (Request *x, Request *y)
{
    if (x->getdelay_time() < y->getdelay_time()) { return -1; }
    else if (x->getdelay_time() > y->getdelay_time()) { return 1; }
    else { return 0; }
}
...
Kernel::Initialize()
{
    ...
    rqlist = new SortedList<Request*>(ReqobjCompare);
